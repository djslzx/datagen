...
def graph_lineage(filename: str, chosen_only=False, ancestors_only=False):
    """
    ...
    G = nx.DiGraph()
    pos = {}  # map from node id to node position (iteration, rank)
    archived_nodes = set()
    # build graph
    for d in data:
        i = d["iteration"]
    ...
    # only visualize ancestors of last generation chosen nodes
    if ancestors_only:
        max_i = max([i for _, _, i in G.edges.data(data="iteration")])
        last_gen = [v for _, v, i in G.edges.data(data="iteration") if i == max_i]
        ancestors = []
...
if __name__ == "__main__":
    ...
    # )
    G = graph_lineage("../datasets/code_alpaca_100_nov_100xAll.jsonl", chosen_only=True, ancestors_only=True)

    # count the number of nodes in each generation
    max_i = max([i for _, _, i in G.edges.data(data="iteration")])
    iters = [i for _, _, i in G.edges.data(data="iteration")]

    # plot histogram of counts with counts printed on top of bar
    _, _, bars = plt.hist(iters, bins=max_i+1)
    plt.bar_label(bars)
    plt.show()
